...
线上：<https://crazygang-ai.github.io/tools/>
...
推荐通过 [Corepack](https://nodejs.org/api/corepack.html) 启用 pnpm（Node ≥ 16.13 自带），无需全局安装：

```bash
corepack enable pnpm
```

> 项目使用 `pnpm-lock.yaml` 锁定依赖，请勿用 npm / yarn 安装，否则依赖树不一致。
...
开发服务器默认监听 <http://localhost:5173>，保存即热更新。

### 脚本

| 命令 | 用途 |
|---|---|
| `pnpm dev` | 开发服务器（HMR，无 Service Worker） |
| `pnpm test` / `pnpm test:watch` | Vitest 单测，CI 模式 / 监听模式 |
| `pnpm lint` | ESLint（flat config） |
| `pnpm format` | Prettier 写回 `src/**` |
| `pnpm build` | 类型检查 + 生产构建至 `dist/` |
| `pnpm preview` | 本地静态服务器跑 `dist/`，验证 PWA / Service Worker 的唯一方式 |

## 部署

主仓库通过 `.github/workflows/deploy.yml` 推送到 `main` 时自动部署到 GitHub Pages。

想 fork 一份部署到自己账号？参见 [docs/DEPLOY.md](./docs/DEPLOY.md)。
